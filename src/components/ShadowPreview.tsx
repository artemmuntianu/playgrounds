...
import { computeSunLightTarget, renderSunDisc } from '../lib/lighting';
...
export const ShadowPreview: React.FC<ShadowPreviewProps> = ({
  ...
  const drawSchematicSunOverlay = (
    ...
    const lt = computeSunLightTarget(sol, cameraAzimuthDeg, cameraFovDeg, width, height, 0);

    if (lt.inView) {
      renderSunDisc(ctx, lt, width, height);
      return;
    }
    ...
    const side = lt.screenDir.dx > 0 ? 'Right ➔' : '⬅ Left';
    ctx.fillText(
      `☀️ Sun: ${lt.isBehind ? 'Behind Camera' : side}`,
      badgeX + 10,
      badgeY + 20
    );
